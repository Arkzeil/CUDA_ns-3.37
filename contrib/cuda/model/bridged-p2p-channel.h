#ifndef BRIDGED_P2P_CHANNEL_H
#define BRIDGED_P2P_CHANNEL_H
...
class BridgedPointToPointNetDevice;
class Packet;
 ...
 * \see TransmitStart
 */
class BridgedP2PChannel : public PointToPointChannel
{
  public:
    /**
    ...
    /**
     * \brief Create a BridgedP2PChannel
     *
     ...
     */
    void Attach(Ptr<BridgedPointToPointNetDevice> device);

    /**
     * \brief Transmit a packet over this channel
     * \param p Packet to transmit
     * \param src Source BridgedPointToPointNetDevice
     * \param txTime Transmit time to apply
     * \returns true if successful (currently always true)
     */
    virtual bool TransmitStart(Ptr<const Packet> p, Ptr<BridgedPointToPointNetDevice> src, Time txTime);
    ...
    /**
     * \brief Get BridgedPointToPointNetDevice corresponding to index i on this channel
     * \param i Index number of the device requested
     * \returns Ptr to BridgedPointToPointNetDevice requested
     */
    Ptr<BridgedPointToPointNetDevice> GetPointToPointDevice(std::size_t i) const;
  ...
  protected:
     ...
     * \returns Ptr to BridgedPointToPointNetDevice source for the
     * specified link
     */
    Ptr<BridgedPointToPointNetDevice> GetSource(uint32_t i) const;

    /**
     * \brief Get the net-device destination
     * \param i the link requested
     * \returns Ptr to BridgedPointToPointNetDevice destination for
     * the specified link
     */
    Ptr<BridgedPointToPointNetDevice> GetDestination(uint32_t i) const;
  ...
  private:
    ...
    /**
     * \brief Wire model for the BridgedP2PChannel
     */
    class Link
    {
      public:
        ...
        Ptr<BridgedPointToPointNetDevice> m_src; //!< First NetDevice
        Ptr<BridgedPointToPointNetDevice> m_dst; //!< Second NetDevice
    };
